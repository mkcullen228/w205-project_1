...
LEFT OUTER JOIN model_images mi ON (m.model_id = cast(mi.model_id as int) AND m.default_price_group = cast(mi.model_price_group as int))
LEFT OUTER JOIN model_features mft ON (m.model_id = cast(mft.model_id as int) AND m.default_price_group = cast(mft.model_price_group as int))
LEFT OUTER JOIN model_attributes ma ON (m.model_id = cast(ma.model_id as int) AND m.default_price_group = cast(ma.model_price_group as int))
LEFT OUTER JOIN model_facets mfa ON (m.model_id = cast(mfa.model_id as int) AND m.default_price_group = cast(mfa.model_price_group as int))
